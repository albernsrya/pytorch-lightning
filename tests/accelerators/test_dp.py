...
def test_dp_raise_exception_with_batch_transfer_hooks(tmpdir, monkeypatch):
    ...
    with pytest.raises(
        ...
        match=r"Overriding `transfer_batch_to_device` is not .* in DP",
    ):
        trainer.fit(model)
    ...
    with pytest.raises(
        MisconfigurationException,
        match=r"Overriding `on_before_batch_transfer` is not .* in DP",
    ):
        trainer.fit(model)
    ...
    with pytest.raises(
        MisconfigurationException,
        match=r"Overriding `on_after_batch_transfer` is not .* in DP",
    ):
        trainer.fit(model)
