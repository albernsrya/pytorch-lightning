...
def test_recursive_application_to_collection():
    ...
    expected_result = {
        ...
        "g": 24.0,
        "h": Feature(
            input_ids=torch.tensor([2.0, 4.0, 6.0]),
    ...
    assert all([isinstance(reduced[k], type(expected_result[k]))
                for k in to_reduce.keys()]), "At least one type was not correctly preserved"
    ...
    assert dataclasses.is_dataclass(
        reduced["h"]
    ) and not isinstance(reduced["h"], type), "Reduction of a dataclass should result in a dataclass"
    ...
    assert dataclasses.is_dataclass(
        reduced["i"]
    ) and not isinstance(reduced["i"], type), "Reduction of a dataclass should result in a dataclass"
