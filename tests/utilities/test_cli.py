...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            dict(auto_lr_find=True, auto_scale_batch_size=True),
        ),
        (
        ...
        (
            ...
            dict(auto_lr_find=True, auto_scale_batch_size=False),
        ),
...
def test_init_from_argparse_args(cli_args, extra_args):
    ...
    with pytest.raises(
        TypeError,
        match=r"__init__\(\) got an unexpected keyword argument 'unknown_arg'",
    ):
...
def test_lightning_cli_args_callbacks(tmpdir):
    ...
    with mock.patch("sys.argv", ["any.py", f"--trainer.callbacks={json.dumps(callbacks)}"]):
        cli = LightningCLI(
            TestModel,
            trainer_defaults=dict(default_root_dir=str(tmpdir), fast_dev_run=True),
        )
...
def test_lightning_cli_args_cluster_environments(tmpdir):
    ...
    class TestModel(BoringModel):
        ...
        def on_fit_start(self):
            ...
            assert isinstance(
                self.trainer.accelerator_connector._cluster_environment,
                SLURMEnvironment,
            )
            self.trainer.ran_asserts = True
...
@pytest.mark.skipif(
    torchvision_version < version.parse("0.8.0"),
    reason="torchvision>=0.8.0 is required",
)
