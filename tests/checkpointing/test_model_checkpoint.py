...
def test_model_checkpoint_score_and_ckpt_val_check_interval(
    ...
    assert (
        len(ckpt_files) == len(model.scores) + additional_ckpt == per_epoch_val_checks * max_epochs + additional_ckpt
    )
    assert len(lr_scheduler_debug) == max_epochs
...
def test_configure_model_checkpoint(tmpdir):
    ...
    with pytest.raises(
        MisconfigurationException,
        match="checkpoint_callback=False but found ModelCheckpoint",
    ):
