...
def test_dataloaders_with_limit_num_batches(tmpdir, limit_train_batches, limit_val_batches, limit_test_batches):
    ...
    if limit_train_batches > 0:
        ...
        for (
            dataloader_idx,
            num_batches,
        ) in trainer.dev_debugger.num_seen_val_check_batches.items():
        ...
        for (
            dataloader_idx,
            num_batches,
        ) in trainer.dev_debugger.num_seen_test_check_batches.items():
            if limit_test_batches > 1e10:
...
def test_dataloaders_with_fast_dev_run(tmpdir, fast_dev_run):
    ...
    if fast_dev_run == "temp":
        ...
        num_val_dataloaders = len(model.val_dataloader())
        assert (
            trainer.dev_debugger.num_seen_sanity_check_batches == trainer.num_sanity_val_steps * num_val_dataloaders
        )
...
def test_warning_with_few_workers_multi_loader(_, tmpdir, ckpt_path, stage):
    ...
    train_multi_dl = {
        ...
        "c_d_e": [train_dl, train_dl, train_dl],
    }
...
def test_warning_with_small_dataloader_and_logging_interval(tmpdir):
    ...
    with pytest.warns(
        UserWarning,
        match=r"The number of training samples \(10\) is smaller than the logging interval",
    ):
        trainer = Trainer(
            default_root_dir=tmpdir,
            max_epochs=1,
    ...
    with pytest.warns(
        UserWarning,
        match=r"The number of training samples \(1\) is smaller than the logging interval",
    ):
        trainer = Trainer(
            default_root_dir=tmpdir,
            max_epochs=1,
...
def test_correct_dataloader_idx_in_hooks(tmpdir, multiple_trainloader_mode):
    ...
    class CustomBoringModel(BoringModel):
        ...
        def train_dataloader(self):
            return {
                "a": DataLoader(RandomDataset(32, 64)),
                "b": DataLoader(RandomDataset(32, 64)),
            }
