...
class BaseFinetuning(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        for (
            opt_idx,
            optimizer,
        ) in trainer.fit_loop.training_loop.batch_loop.get_active_optimizers():
...
class BackboneFinetuning(BaseFinetuning):
    ...
    def finetune_function(
        ...
        if epoch == self.unfreeze_backbone_at_epoch:
            ...
            initial_backbone_lr = (
                self.backbone_initial_lr if self.backbone_initial_lr is not None else current_lr *
                self.backbone_initial_ratio_lr
            )
            ...
            next_current_backbone_lr = (
                current_lr if
                (self.should_align and next_current_backbone_lr > current_lr) else next_current_backbone_lr
            )
